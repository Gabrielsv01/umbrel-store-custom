...
function CategoryBadge({ category }: { category: string }) {
  ...
  return <span className={`rounded-full px-2 py-0.5 text-[11px] ${color}`}>{category}</span>
}
...
export default function CatalogModal({ onClose, onSelect }: CatalogModalProps) {
  ...
  useEffect(() => {
    fetchCatalog()
      .then(setCatalog)
      .catch((err: unknown) => setError(err instanceof Error ? err.message : 'Failed to load catalog'))
      ...
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-gray-800 bg-gray-900">
        <div className="flex items-center justify-between border-b border-gray-800 px-6 py-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-400">Select a template to pre-fill the deploy form</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            <div className="flex items-center justify-center py-12 text-gray-500">Loading catalog...</div>
          ) : error ? (
            <div className="flex items-center justify-center py-12 text-red-400">{error}</div>
          ) : filtered.length === 0 ? (
            <div className="flex items-center justify-center py-12 text-gray-500">No results found.</div>
          ) : (
              ...
              {filtered.map((entry) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className="font-semibold text-white group-hover:text-blue-300">{entry.name}</span>
                    <CategoryBadge category={entry.category} />
                  </div>
                  <p className="text-xs text-gray-400">{entry.description}</p>
                  <div className="flex items-center gap-2 text-[11px] text-gray-500">
                    <span className="rounded bg-gray-800 px-1.5 py-0.5 font-mono">{entry.transport}</span>
                    {entry.secretKeys && entry.secretKeys.length > 0 && (
                      <span className="text-yellow-500">🔒 requires API key</span>
                    )}
                  </div>
                </button>
              ))}
