...
import type { CustomToolDefinition, CustomToolMethod, SharedVolumeAccess, DockerContainerToolsAccess } from '../types/customTools';
...
import SharedVolumeAccessManager from './customTools/SharedVolumeAccessManager';
import DockerContainerToolsManager from './customTools/DockerContainerToolsManager';
...
export default function CustomToolsCreator() {
  ...
  const handleSharedVolumeAccessChange = (accesses: SharedVolumeAccess[]) => {
    setDefinition({ ...definition, sharedVolumeAccess: accesses });
  };

  const handleDockerContainerToolsChange = (tools: DockerContainerToolsAccess | undefined) => {
    setDefinition({ ...definition, dockerContainerTools: tools });
  };

  const getAutoGeneratedTools = () => {
    if (!definition.sharedVolumeAccess || definition.sharedVolumeAccess.length === 0) {
      return [];
    }

    const hasReadAccess = definition.sharedVolumeAccess.some(a => a.canRead);
    const hasWriteAccess = definition.sharedVolumeAccess.some(a => a.canWrite);
    const hasDeleteAccess = definition.sharedVolumeAccess.some(a => a.canDelete);

    return [
      'list_shared_files',
      ...(hasReadAccess ? ['read_shared_file', 'read_shared_file_base64'] : []),
      ...(hasWriteAccess ? ['write_shared_file'] : []),
      ...(hasDeleteAccess ? ['delete_shared_file'] : []),
    ];
  };
  ...
  const hasCustomMethods = definition.methods.length > 0;
  const hasSharedVolume = definition.sharedVolumeAccess && definition.sharedVolumeAccess.length > 0;
  const hasDockerContainerTools = definition.dockerContainerTools && definition.dockerContainerTools.commands.length > 0;
  const isFormValid =
    definition.name.trim().length > 0 &&
    (hasCustomMethods || hasSharedVolume || hasDockerContainerTools);
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      {/* Shared Volume Access Section */}
      <div className="mt-8 rounded-lg border border-gray-800 bg-gray-900 p-6">
        <h2 className="text-lg font-semibold text-white">Shared Volume Access</h2>
        <p className="mt-2 text-sm text-gray-400">
          Configure which shared folders this tool can access and what permissions it has.
        </p>

        <div className="mt-6">
          <SharedVolumeAccessManager
            accesses={definition.sharedVolumeAccess || []}
            onChange={handleSharedVolumeAccessChange}
          />
        </div>

        {getAutoGeneratedTools().length > 0 && (
          <div className="mt-6 rounded border border-blue-700/50 bg-blue-900/20 p-4">
            <h3 className="text-sm font-semibold text-blue-300">
              📦 Auto-Generated Tools
            </h3>
            <p className="mt-2 text-xs text-gray-400">
              These tools will be automatically generated based on your shared volume configuration:
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              {getAutoGeneratedTools().map((tool) => (
                <span
                  key={tool}
                  className="rounded-full bg-blue-600/30 px-3 py-1 text-xs text-blue-300 border border-blue-600/50"
                >
                  {tool}
                </span>
              ))}
      ...
      {/* Docker Container CLI Tools Section */}
      <div className="mt-8 rounded-lg border border-gray-800 bg-gray-900 p-6">
        <h2 className="text-lg font-semibold text-white">Docker Container CLI Tools</h2>
        <p className="mt-2 text-sm text-gray-400">
          Expose CLI commands from a running container as MCP tools. Each command will be automatically wrapped as a callable tool.
        </p>

        <div className="mt-6">
          <DockerContainerToolsManager
            containerTools={definition.dockerContainerTools}
            onChange={handleDockerContainerToolsChange}
          />
        </div>

        {definition.dockerContainerTools && definition.dockerContainerTools.commands.length > 0 && (
          <div className="mt-6 rounded border border-green-700/50 bg-green-900/20 p-4">
            <h3 className="text-sm font-semibold text-green-300">
              🔧 Auto-Generated Container Tools
            </h3>
            <p className="mt-2 text-xs text-gray-400">
              These tools will be automatically generated from your container commands:
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              {definition.dockerContainerTools.commands.map((cmd) => (
                <span
                  key={cmd.toolName}
                  className="rounded-full bg-green-600/30 px-3 py-1 text-xs text-green-300 border border-green-600/50 font-mono"
                >
                  {cmd.toolName}
                </span>
              ))}
      ...
      <div className="mt-8 rounded-lg border border-gray-800 bg-gray-900 p-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">Custom Methods</h2>
          <button
        ...
        <p className="mt-2 text-xs text-gray-500">
          💡 Optional: Add custom methods or use only the auto-generated shared volume tools above
        </p>

        {definition.methods.length === 0 ? (
          <div className="mt-6 rounded border border-gray-700 bg-gray-800/50 p-4 text-center">
            <p className="text-sm text-gray-400">
              {getAutoGeneratedTools().length > 0
                ? '✓ No custom methods needed - you have auto-generated shared volume tools'
                : 'No custom methods added yet. Click "Add Method" or configure Shared Volume Access above.'}
            </p>
          </div>
        ) : (
      ...
      {!definition.name.trim() && (
        <div className="mt-6 rounded-lg border border-yellow-700 bg-yellow-900/30 p-3">
          <p className="text-sm text-yellow-300">
            ⚠️ Add an MCP name to continue
          </p>
        </div>
      )}
      {definition.name.trim() && !hasCustomMethods && !hasSharedVolume && !hasDockerContainerTools && (
        <div className="mt-6 rounded-lg border border-yellow-700 bg-yellow-900/30 p-3">
          <p className="text-sm text-yellow-300">
            ⚠️ Add custom methods, configure shared volume access, or select docker container tools
          </p>
        </div>
      )}
      ...
      <div className="mt-8 flex gap-4">
        <button
          ...
          title={!isFormValid ? 'Complete the form to deploy' : ''}
        >
